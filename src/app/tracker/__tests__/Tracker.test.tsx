...
describe('Testing Combatant Info Input 2', () => {
  it('Tests entering four new characters', async () => {
    ...
    const charName1 = screen.getAllByLabelText(/character name/i)
    ...
    const charInit1 = screen.getAllByLabelText(/initiative/i)
    expect(charInit1.length).toBe(2)
    expect(charInit1[1].value).toBe('10')

    const charCurHp1 = screen.getAllByLabelText(/current hp/i)
    expect(charCurHp1.length).toBe(2)
    expect(charCurHp1[1].value).toBe('7')

    const charMaxHp1 = screen.getAllByLabelText(/max hp/i)
